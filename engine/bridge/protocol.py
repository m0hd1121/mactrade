...
write plain files inside its own sandboxed `MQL5/Files/` (and shared
`MQL5/Files/Common/`) directory. This project therefore uses that shared
folder as a simple, dependency-free, polling message bus between the EA
(inside MT5) and this Python engine (outside MT5). No Wine, no VPS, no
Windows, no DLLs.

Layout, all inside <MT5 Common Data Path>/Files/ForexTradingSystem/:
